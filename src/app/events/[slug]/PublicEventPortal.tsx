...
// 🚀 Imported the standalone registration component container loop
...
export default function PublicEventPortal({ event }: PublicEventPageProps) {
  const [isDark, setIsDark] = useState(false); // Defaulted to false to maintain light theme priority
  ...
  return (
    <div className={`min-h-screen font-sans selection:bg-blue-500 selection:text-white transition-colors duration-300 ${isDark ? 'bg-[#020617] text-slate-100 dark' : 'bg-slate-50 text-slate-800'}`}>
      ...
      <div className={`relative w-full h-[30vh] md:h-[45vh] overflow-hidden border-b ${isDark ? 'border-white/5' : 'border-slate-200'}`}>
        <img src={event.coverImageUrl} alt="Event Canvas Cover" className="w-full h-full object-cover filter brightness-[0.5] saturate-[1.1]" />
        <div className={`absolute inset-0 bg-gradient-to-t via-transparent to-black/40 ${isDark ? 'from-[#020617]' : 'from-slate-50'}`} />
        
        {/* Floating Utility Actions Row */}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
          ...
          <div className="lg:col-span-2 space-y-8">
            <div className={`p-6 md:p-8 backdrop-blur-xl rounded-3xl border shadow-2xl space-y-5 ${isDark ? 'bg-[#070f2e]/60 border-white/10 shadow-black/40' : 'bg-white/80 border-slate-200 shadow-slate-200/50'}`}>
              <div className="flex flex-wrap items-center gap-2.5">
                <span className={`px-3 py-1 rounded-lg text-[9px] font-black uppercase tracking-widest border ${theme.lightBg} ${theme.text} ${theme.border}`}>{event.type}</span>
                <span className={`px-3 py-1 rounded-lg text-[9px] font-black uppercase tracking-widest border ${isDark ? 'bg-white/5 border-white/10 text-slate-400' : 'bg-slate-100 border-slate-200 text-slate-500'}`}>{event.protocol.replace('-', ' ')}</span>
              </div>
              ...
              <div className="space-y-2">
                ...
                <p className={`text-base font-medium border-l-2 pl-4 italic ${isDark ? 'text-slate-400 border-slate-700' : 'text-slate-600 border-slate-300'}`}>"{event.tagline}"</p>
              </div>

              {/* 🚀 NEW ADDITION: Inline Event Description Context Block */}
              ...
              <div className={`grid grid-cols-1 sm:grid-cols-2 gap-4 pt-4 border-t text-sm ${isDark ? 'border-white/5 text-slate-300' : 'border-slate-100 text-slate-600'}`}>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className={`text-[10px] uppercase font-bold tracking-wider ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>Geographic Anchor</p>
                    <p className="font-semibold truncate max-w-[240px]">{event.venueName}</p>
                  </div>
                </div>
              </div>
            </div>

            <div className={`p-6 md:p-8 border rounded-3xl space-y-4 ${isDark ? 'bg-[#020617] border-white/5' : 'bg-white border-slate-200/60'}`}>
              <h3 className={`text-sm font-black uppercase tracking-wider ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>Experience Scope Parameters</h3>
              <p className={`text-sm leading-relaxed whitespace-pre-line ${isDark ? 'text-slate-300' : 'text-slate-600'}`}>{event.description}</p>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div className={`p-5 border rounded-2xl text-center space-y-1 ${isDark ? 'bg-white/5 border-white/5' : 'bg-white border-slate-200'}`}>
                ...
                <h5 className={`text-[10px] uppercase font-black tracking-widest ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>Reporting Loop</h5>
                <p className={`text-xs font-bold ${isDark ? 'text-slate-200' : 'text-slate-800'}`}>09:30 AM IST</p>
              </div>
              <div className={`p-5 border rounded-2xl text-center space-y-1 ${isDark ? 'bg-white/5 border-white/5' : 'bg-white border-slate-200'}`}>
              ...
              <div className={`p-5 border rounded-2xl text-center space-y-1 ${isDark ? 'bg-white/5 border-white/5' : 'bg-white border-slate-200'}`}>
                ...
                <h5 className={`text-[10px] uppercase font-black tracking-widest ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>Growth Scope</h5>
                <p className={`text-xs font-bold ${isDark ? 'text-slate-200' : 'text-slate-800'}`}>{event.hypeThreshold}+ Attendees</p>
              </div>
            </div>
          ...
          <div className="space-y-6 lg:sticky lg:top-6">
            <div className={`hidden lg:block w-full aspect-[4/5] rounded-3xl border overflow-hidden group relative shadow-2xl ${isDark ? 'border-white/10 shadow-black/40' : 'border-slate-200 shadow-slate-200/40'}`}>
              <img src={event.posterImageUrl} alt="Printable Event Poster" className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700 filter saturate-[1.05]" />
            ...
            <div className={`p-6 md:p-8 border rounded-3xl shadow-2xl relative overflow-hidden flex flex-col justify-between min-h-[420px] ${isDark ? 'bg-[#091133] border-blue-500/20 shadow-black/40' : 'bg-white border-slate-200 shadow-slate-200/30'}`}>
              ...
              {/* STICKY CARD FOOTER ORGANIZER LABEL */}
